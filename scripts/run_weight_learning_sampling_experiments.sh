...
function run_example() {
    ...
    out_directory="${BASE_OUT_DIR}/${srl_model_type}/sampling_study/${example_name}/${wl_method}/${evaluator}/${alpha}/${fold}"
                    ...
                    for ((fold=0; fold<${EXAMPLE_FOLDS[${example_name}]}; fold++)) do
                        if [[ "${SUPPORTED_WL_METHODS[${srl_modeltype}]}" == *"${wl_method}"* ]]; then
                            if [[ "${SUPPORTED_EXAMPLES[${srl_modeltype}]}" == *"${example_name}"* ]]; then
                                run_example "${srl_modeltype}" "${example_directory}" "${wl_method}" "${evaluator}" "${alpha}" "${fold}"
                            fi
                        fi
                    done
                done
